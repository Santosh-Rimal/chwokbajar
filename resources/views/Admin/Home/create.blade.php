<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('Add Home Entry') }}</h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('admin.homes.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300"
                                id="slogan" type="text" name="slogan" value="{{ old('slogan') }}" required>
                            @error('slogan')
                                <span class="text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
